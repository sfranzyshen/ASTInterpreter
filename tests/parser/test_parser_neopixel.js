...
/**
 * Arduino Parser Test - NeoPixel Test Suite
 * Tests parsing of all 2 NeoPixel tests from neopixel.js
 * Parser-only testing (no interpreter execution)
 */

console.log('🚀 Arduino Parser Test - All 2 NeoPixel Examples');
console.log('===============================================');
...
let testFiles;
try {
    const testData = require('../../neopixel.js');
    testFiles = testData.neopixelFiles || testData;
} catch (error) {
    console.error('❌ Failed to load neopixel.js:', error.message);
    process.exit(1);
}

console.log('✅ Dependencies loaded:', testFiles.length, 'NeoPixel tests');
...
async function runAllTests() {
    ...
    // Final summary
    console.log('\n' + '='.repeat(50));
    console.log('📊 FINAL RESULTS - ALL 2 NEOPIXEL TESTS (PARSER)');
