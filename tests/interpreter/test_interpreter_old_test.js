...
/**
 * Working Arduino Interpreter Test - old_test.js
 * Tests comprehensive test cases using the proven pattern
 */

console.log('🚀 Arduino Interpreter Test - old_test.js');
console.log('==========================================');
...
console.log('✅ Dependencies loaded:', testFiles.length, 'tests from old_test.js');
...
function testExample(test, index) {
    return new Promise((resolve) => {
        ...
        try {
            ...
            const timeoutDuration = 5000; // 5 seconds for comprehensive tests
            ...
            const checkCompletion = () => {
                if (executionCompleted) {
                    ...
                    if (executionError) {
                        ...
                        console.log(`  ❌ FAILED: ${errorStr.substring(0, 120)}...`);
                        resolve({
                            success: false,
                            name: testName,
                            error: errorStr,
...
async function runAllTests() {
    ...
    // Final summary
    console.log('\n' + '='.repeat(50));
    console.log('📊 FINAL RESULTS - OLD_TEST.JS COMPREHENSIVE TESTS');
    ...
    if (failures.length > 0 && failures.length <= 15) {
        ...
        console.log(`\n🔍 First 15 failures shown, ${failures.length - 15} more not displayed`);
        failures.slice(0, 15).forEach((failure, idx) => {
            console.log(`${idx + 1}. ${failure.name}: ${failure.error.substring(0, 60)}...`);
        });
    }
