...
const TokenType = {
    ...
    STAR_EQUAL: 'STAR_EQUAL', SLASH_EQUAL: 'SLASH_EQUAL', PERCENT_EQUAL: 'PERCENT_EQUAL',
...
class Token {
    ...
    toString() {
        return `Token(${this.type}, ${this.value}, L${this.line})`;
    }
}
...
class Lexer {
    ...
    number() {
        let result = '';
        if (this.currentChar === '0' && (this.peek() === 'x' || this.peek() === 'X')) { // Hexadecimal
            result += this.currentChar; this.advance();
            result += this.currentChar; this.advance();
            while (this.currentChar !== null && this._isHexDigit(this.currentChar)) {
                result += this.currentChar; this.advance();
            }
            return new Token(TokenType.NUMBER, parseInt(result), this.line);
        }
        
        if (this.currentChar === '0' && (this.peek() === 'b' || this.peek() === 'B')) { // Binary
            result += this.currentChar; this.advance();
            result += this.currentChar; this.advance();
            while (this.currentChar !== null && (this.currentChar === '0' || this.currentChar === '1')) {
                result += this.currentChar; this.advance();
            }
            return new Token(TokenType.NUMBER, parseInt(result.substring(2), 2), this.line);
        }


        if (this.currentChar === '0' && this._isOctalDigit(this.peek())) { // Octal
             while (this.currentChar !== null && this._isOctalDigit(this.currentChar)) {
                result += this.currentChar; this.advance();
            }
            return new Token(TokenType.NUMBER, parseInt(result, 8), this.line);
        }
        ...
        return new Token(TokenType.NUMBER, parseFloat(result), this.line);
    }
    ...
    getNextToken() {
        while (this.currentChar !== null) {
            ...
            switch(this.currentChar) {
                ...
                case '#': token = new Token(TokenType.HASH, '#', this.line); break;
                case '?': token = new Token(TokenType.QUESTION, '?', this.line); break;
                case ':': token = new Token(TokenType.COLON, ':', this.line); break;
                default:
                    console.error(`Lexer error: Unrecognized character '${this.currentChar}' on line ${this.line}`);
                    this.advance();
                    continue;
            }
            this.advance();
...
class VarDeclNode extends ASTNode { constructor(type, identifier, value) { super(); this.type = type; this.identifier = identifier; this.value = value; } }
...
class ArrayInitializerNode extends ASTNode { constructor(elements) { super(); this.elements = elements; } }
...
class DeclaratorNode extends ASTNode { constructor(identifier, pointer, arraySpecifiers) { super(); this.identifier = identifier; this.pointer = pointer; this.arraySpecifiers = arraySpecifiers; } }
...
class Parser {
    ...
    declaration() {
        ...
        if (this.currentToken.type === TokenType.LPAREN) {
            ...
            try {
                if (!(declarator.identifier instanceof VarNode) && !(declarator.identifier instanceof NoOpNode)) {
                     const funcDef = this.func_declaration(type, declarator.identifier);
                     const value = this.initializer();
                     this.eat(TokenType.SEMICOLON);
                     const newDeclarator = new DeclaratorNode(funcDef.name, declarator.pointer, declarator.arraySpecifiers);
                     return new VarDeclNode(type, newDeclarator, value);
                }
                return this.func_declaration(type, declarator);
            } catch (e) {
                if (e instanceof ParserError) { // Only catch parser errors for backtracking
        ...
        let value = this.initializer();
        declarations.push(new VarDeclNode(type, currentDeclarator, value));
    ...
    declaration_specifiers() {
        ...
        const specifierTokens = [
            TokenType.CONST, TokenType.STATIC, TokenType.EXTERN, TokenType.TYPEDEF, TokenType.VOLATILE,
            TokenType.VOID, TokenType.INT, TokenType.FLOAT, TokenType.CHAR, TokenType.BOOLEAN, 
            TokenType.BOOL, TokenType.LONG, TokenType.SHORT, TokenType.BYTE, TokenType.WORD, 
            TokenType.UNSIGNED, TokenType.STRUCT
        ];

        while (specifierTokens.includes(this.currentToken.type)) {
            let specifierText = this.currentToken.value;
            let specifierToken = this.currentToken;
            ...
            if (specifierToken.type === TokenType.STRUCT && this.currentToken.type === TokenType.IDENTIFIER) {
                ...
                this.currentToken.type === TokenType.LONG ||
        ...
        if (specifiers.length === 0) {
            if (this.currentToken.type === TokenType.IDENTIFIER) {
                specifiers.push(this.currentToken.value);
                this.eat(TokenType.IDENTIFIER);
            } else {
        ...
        return new TypeNode(new Token(TokenType.IDENTIFIER, specifiers.join(' '), this.currentToken.line));
    }
    ...
    declarator() {
        ...
        return new DeclaratorNode(identifier, pointer, arraySpecifiers);
    }
    ...
    var_declaration(type, declarator, isConst) {
        let value = this.initializer();
        const varDecl = new VarDeclNode(type, declarator, value);
    ...
    func_declaration(returnType, declarator) {
        ...
        if (this.currentToken.type === TokenType.SEMICOLON) {
            this.eat(TokenType.SEMICOLON);
            return new FuncDefNode(returnType, declarator, params, null);
        }

        const body = this.compound_statement();
        return new FuncDefNode(returnType, declarator, params, body);
    }
    ...
    parameter() {
        ...
        if (this.currentToken.type === TokenType.LPAREN) {
            ...
            const funcSignature = new FuncDefNode(type, declarator, params, null);
            declarator = new DeclaratorNode(funcSignature, null, []);
        }
    ...
    statement() {
        ...
        if (typeAndSpecifierTokens.includes(this.currentToken.type) || this.currentToken.type === TokenType.IDENTIFIER) {
            ...
            try {
                return this.declaration();
                 ...
                 if (e instanceof ParserError) {
                    ...
                    throw e; // Re-throw non-parser errors
                }
            }
        }
    ...
    for_statement() {
        ...
        this.eat(TokenType.LPAREN);
        const init = this.currentToken.type === TokenType.SEMICOLON ? new NoOpNode() : this.statement();
...
class Interpreter {
    ...
    visit_ParamNode(node) {
        ...
        } else if (node.declarator.identifier instanceof FuncDefNode) {
            // It's a function pointer, find the name inside
            let innerDeclarator = node.declarator.identifier.name;
            while(!(innerDeclarator.identifier instanceof VarNode)) {
                innerDeclarator = innerDeclarator.identifier;
            }
            paramName = innerDeclarator.identifier.value;
        }
        else {
    ...
    visit_VarDeclNode(node) {
        ...
        if (!typeSymbol && !typeName.startsWith('struct') && !typeName.startsWith('typedef')) {
    ...
    // Add placeholder visitors for nodes that don't need semantic checks yet
    visit_NoOpNode(node) { }
    visit_NumberNode(node) { }
    visit_CharLiteralNode(node) { }
    visit_StringNode(node) { }
    visit_ArrayInitializerNode(node) { node.elements.forEach(el => this.visit(el)); }
