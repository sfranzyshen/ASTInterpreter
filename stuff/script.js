...
async function runInterpreter(code) {
    ...
    try {
        ...
        logToConsole("--- Abstract Syntax Tree (Parser Pass Complete) ---");
        const astString = prettyPrintAST(ast);
        logToConsole(astString);
        
        logToConsole("--- Starting Semantic Analysis ---");
        const interpreter = new Interpreter(ast);
        interpreter.interpret(); // This will walk the tree and build the symbol table

        logToConsole("--- Global Symbol Table ---");
        // A simple way to visualize the symbol table for now
        logToConsole(JSON.stringify(interpreter.symbolTable.symbols, null, 2));
        logToConsole("--- Semantic Analysis Complete ---");


    } catch (error) {
        // Check if it's our custom parser error or a generic one
        if (error instanceof ParserError || error instanceof SemanticError) {
