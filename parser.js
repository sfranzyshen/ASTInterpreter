 ...
 * Arduino/C++ Parser 
 * 
 * A comprehensive parser for Arduino/C++ code with enhanced error handling.
 * 
 * USAGE:
 *   // Basic parsing
 *   const ast = parse(code);
 * 
 *   // Verbose output with error analysis
 *   const ast = parse(code, { verbose: true });
 * 
 *   // Throw on critical errors (instead of graceful recovery)
 *   const ast = parse(code, { throwOnError: true });
 * 
 * FEATURES:
 *   ✅ Function definitions and declarations
 *   ✅ Struct declarations with self-references
 *   ✅ typedef support
 *   ✅ Bit-fields (unsigned int a : 1;)
 *   ✅ Pointer declarations (*ptr, struct Node*)
 *   ✅ Storage class specifiers (static, extern, volatile, const)
 *   ✅ Enhanced type parsing (unsigned int, struct TypeName)
 *   ✅ Comprehensive error recovery and reporting
 *   ✅ Success rate analysis and suggestions
 * 
 * CHANGELOG v3.0.0:
 *   + Added function forward declarations
 *   + Added typedef struct support
 *   + Added self-referential struct support
 *   + Added bit-field parsing
 *   + Enhanced error messaging with context
 *   + Added verbose analysis mode
...
// Import Arduino Preprocessor - will be resolved when needed
function getArduinoPreprocessor() {
    try {
        if (typeof require !== 'undefined') {
            // Node.js environment
            const preprocessorModule = require('./preprocessor.js');
            return preprocessorModule.ArduinoPreprocessor;
        } else if (typeof window !== 'undefined' && window.ArduinoPreprocessor) {
            // Browser environment (preprocessor loaded separately)
            return window.ArduinoPreprocessor;
        }
    } catch (error) {
        // Fallback if preprocessor not available
        if (typeof console !== 'undefined') {
            console.warn('Arduino Preprocessor not available:', error.message);
        }
    }
    return null;
}
...
class Parser {
    ...
    parse() {
        ...
        while (this.currentToken.type !== 'EOF') {
            try {
                programNode.children.push(this.parseTopLevelStatement());
            } catch (e) {
...
// prettyPrintAST remains unchanged but will now support the new node types
...
(function(globalContext) {
    globalContext.parse = parse;
    globalContext.prettyPrintAST = prettyPrintAST;
    globalContext.Parser = Parser;
    globalContext.PARSER_VERSION = PARSER_VERSION;
})(typeof window !== 'undefined' ? window : (typeof global !== 'undefined' ? global : this));

// Node.js module export
if (typeof module !== 'undefined' && module.exports) {
    module.exports = { Parser, parse, prettyPrintAST };
}